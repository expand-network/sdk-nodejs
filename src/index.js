...
const { getSignatureDutchOrder } = require("./uniswapX/singer");
...
exports.signTransaction = async (transactionObject, options) => {
  ...
  const chainId = await common.getChainId({
    ...
    chainSymbol: filterOptions.chainSymbol,
  });
  ...
  const web3 = await initialiseWeb3({
    rpc: filterOptions.rpc,
    chainId,
  ...
  const rawData = await rawTransaction[
    `signTransaction${filterOptions.chainName}`
  ](web3, transactionObject, options);
...
exports.signUniswapXDutchOrder = async (options) => {
  const configuration = {};
  const filterOptions = options;
  filterOptions.function = "signUniswapXDutchOrder()";
  const validJson = await schemaValidator.validateInput(options);
  ...
  const rpc = await axios.get(apiURL, configuration);
  filterOptions.rpc = rpc.data.data.rpc;
  const signature = await getSignatureDutchOrder(
    filterOptions,
    filterOptions.privateKey
  );
  return signature;
};
